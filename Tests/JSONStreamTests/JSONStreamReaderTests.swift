...
class JSONReaderTests: XCTestCase {
    ...
    func testSimpleObjectAsync() async throws {
        let jsonStream = AsyncStream([UInt8].self) { continutation in
            continutation.yield("{\"the key\": \"the value\"}".utf8.reversed().reversed())
            continutation.yield("{\"the key2\": \"the value\"}".utf8.reversed().reversed())
            continutation.finish()
        }
        let reader = JSONReader(inputStreamAsync: jsonStream, eventStream: eventStream)
        try await reader.start()

        XCTAssertEqual(eventStream.allObjects[0], ["the key": "the value", "the key2": "the value"])
        XCTAssertEqual(eventStream.allObjects[1], ["the key2": "the value"])
    }
    ...
    func testArrayWithExponentialNumbers() throws {
        ...
        XCTAssertEqual(eventStream.allArrays[0], [-0.042, 42_000])
    }
    ...
    func testArrayWithNegativeDoubleExponentialNumber() throws {
        ...
        XCTAssertEqual(eventStream.allArrays[0], [-143_184_751_248_132])
    }
    ...
    func testComplexStream() throws {
        ...
        let firstObjectString = try assertNotNil { try String(
            data: JSONSerialization.data(withJSONObject: dict, options: [.prettyPrinted]),
            encoding: .utf8
        ) }
        let secondObjectString = try assertNotNil { try String(
            data: JSONSerialization.data(withJSONObject: array, options: [.prettyPrinted]),
            encoding: .utf8
        ) }
